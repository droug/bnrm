  ...
  const { data: latestExhibition } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .select("id, slug, title_fr, title_ar, teaser_fr, teaser_ar, cover_image_url")
        .eq("status", "published")
        ...
        <div className="container mx-auto px-4 relative z-10 flex flex-col min-h-[90vh]">
          ...
          <div className="max-w-2xl mx-auto w-full mb-8">
            <div className="relative flex items-center bg-white rounded-full shadow-xl overflow-hidden">
              ...
              />
              <button 
                className="m-1.5 px-6 py-2 border border-gold-bn-primary bg-gold-bn-primary text-white hover:bg-transparent hover:text-gold-bn-primary rounded-full text-sm font-medium transition-colors"
                onClick={() => {
          ...
          <div className="flex-1 flex items-center justify-center py-6">
            {!loadingFeatured && featuredWorks.length > 0 && (
              <div className="relative w-full max-w-5xl mx-auto px-4">
                ...
                <button 
                  className="absolute -left-4 md:-left-8 top-1/2 -translate-y-1/2 z-20 text-gold-bn-primary hover:text-gold-bn-primary-dark transition-colors p-2"
                  onClick={() => {
                ...
                <button 
                  className="absolute -right-4 md:-right-8 top-1/2 -translate-y-1/2 z-20 text-gold-bn-primary hover:text-gold-bn-primary-dark transition-colors p-2"
                  onClick={() => {
                    ...
                    <div className="flex flex-col md:flex-row items-center gap-8">
                      ...
                      <div className="relative flex-shrink-0">
                        <div className="w-48 h-48 md:w-56 md:h-56 rounded-2xl bg-gradient-to-br from-gold-bn-primary to-gold-bn-deep p-1 shadow-2xl shadow-gold-bn-primary/30 group-hover:shadow-gold-bn-primary/50 transition-shadow duration-500">
                          {latestExhibition.cover_image_url ? (
                            <img 
                              src={latestExhibition.cover_image_url} 
                              alt={language === 'ar' ? latestExhibition.title_ar || latestExhibition.title_fr : latestExhibition.title_fr}
