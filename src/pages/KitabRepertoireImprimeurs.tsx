...
import { Printer, Book, MapPin, Mail, Phone, ExternalLink } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { useState } from "react";

const KitabRepertoireImprimeurs = () => {
  ...
  const imprimeurs = [
    {
      ...
      ville: "Casablanca",
      email: "contact@impmod.ma",
      telephone: "+212 5 22 XX XX XX",
      nombrePublications: 320,
      services: ["Impression offset", "Reliure"],
    },
    {
      id: 2,
      nom: "Print Plus",
      ville: "Rabat",
      email: "info@printplus.ma",
      telephone: "+212 5 37 XX XX XX",
      nombrePublications: 245,
      services: ["Impression numérique", "Design"],
    },
    {
      id: 3,
      nom: "Royal Print",
      ville: "Marrakech",
      email: "contact@royalprint.ma",
      telephone: "+212 5 24 XX XX XX",
      nombrePublications: 178,
      services: ["Impression offset", "Emballage"],
    },
  ];

  const filteredImprimeurs = imprimeurs.filter(imprimeur =>
    imprimeur.nom.toLowerCase().includes(searchTerm.toLowerCase()) ||
    imprimeur.ville.toLowerCase().includes(searchTerm.toLowerCase())
  );
      ...
      <main className="flex-1 container mx-auto px-4 py-12">
        ...
        <div className="text-center mb-12 space-y-4">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-[hsl(var(--kitab-primary))]/10 border border-[hsl(var(--kitab-primary))]/20 mb-4">
            <Printer className="h-5 w-5 text-[hsl(var(--kitab-primary))]" />
          ...
          <p className="text-xl text-muted-foreground max-w-3xl mx-auto">
            Tous les imprimeurs ayant rejoint le programme Kitab de la BNRM
          </p>
        </div>
        ...
        <div className="max-w-2xl mx-auto mb-12">
          <Input
            type="search"
            placeholder="Rechercher un imprimeur par nom ou ville..."
            value={searchTerm}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
          <Card className="border-[hsl(var(--kitab-primary))]/20">
            <CardContent className="pt-6 text-center">
              <div className="text-4xl font-bold text-[hsl(var(--kitab-primary))] mb-2">{imprimeurs.length}</div>
              <div className="text-sm text-muted-foreground">Imprimeurs Inscrits</div>
            </CardContent>
          </Card>
          <Card className="border-[hsl(var(--kitab-primary))]/20">
            <CardContent className="pt-6 text-center">
              <div className="text-4xl font-bold text-[hsl(var(--kitab-primary))] mb-2">
                {imprimeurs.reduce((acc, i) => acc + i.nombrePublications, 0)}
              </div>
              <div className="text-sm text-muted-foreground">Publications Imprimées</div>
            </CardContent>
          </Card>
          <Card className="border-[hsl(var(--kitab-primary))]/20">
            <CardContent className="pt-6 text-center">
              <div className="text-4xl font-bold text-[hsl(var(--kitab-primary))] mb-2">
                {new Set(imprimeurs.map(i => i.ville)).size}
              </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredImprimeurs.map((imprimeur) => (
            <Card key={imprimeur.id} className="group hover:shadow-xl transition-all duration-300 border-[hsl(var(--kitab-primary))]/20 hover:border-[hsl(var(--kitab-primary))]/40">
              <CardHeader>
                <div className="flex items-start justify-between mb-2">
                  <div className="p-3 rounded-lg bg-[hsl(var(--kitab-primary))]/10 group-hover:bg-[hsl(var(--kitab-primary))]/20 transition-colors">
                    <Printer className="h-6 w-6 text-[hsl(var(--kitab-primary))]" />
                  </div>
                  <span className="text-sm font-medium text-[hsl(var(--kitab-primary))]">
                    {imprimeur.nombrePublications} publications
                  </span>
                </div>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {imprimeur.services.map((service, index) => (
                    <span
                      key={index}
        ...
        {filteredImprimeurs.length === 0 && (
          <div className="text-center py-12">
            ...
            <p className="text-lg text-muted-foreground">Aucun imprimeur trouvé</p>
          </div>
        )}
      </main>
...
export default KitabRepertoireImprimeurs;