...
export function DepositValidationWorkflow() {
  ...
  // États pour l'attribution de numéros
  ...
  const defaultSettings: Record<string, { isbn: boolean; issn: boolean; ismn: boolean; dl: boolean }> = {
  ...
  const getDepositTypeLabel = (type: string) => {
    switch (type) {
      ...
      case 'monographie_electronique_collection': return 'Monographie Élec. (Collection)';
  ...
  const openNumberSelection = async (numberType: 'isbn' | 'issn' | 'ismn') => {
    ...
    // Fetch available ranges for this type
    try {
    ...
    <>
                              ...
                              <TableRow key={request.id}>
                                ...
                                <TableCell>
                                  <div className="flex items-center gap-2">
                                    ...
                                    {activeTab === "pending" && !hasAnyAttributedNumber(request) && (
                                      <Button
                                        size="sm"
                                        onClick={() => {
      ...
      <Dialog open={isAttributionDialogOpen} onOpenChange={setIsAttributionDialogOpen}>
        <DialogContent className="max-w-lg">
          ...
          {attributionRequest && (() => {
            const depositCategory = getDepositCategory(attributionRequest);
            const settings = getSettingsForType(depositCategory);
            return (
              <div className="space-y-4">
                <div className="bg-muted p-4 rounded-lg space-y-2">
                  ...
                  <div>
                    ...
                    <div>{getDepositTypeLabel(depositCategory)}</div>
                  </div>
                </div>
