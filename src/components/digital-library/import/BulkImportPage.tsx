...
export default function BulkImportPage({ onSuccess }: BulkImportPageProps) {
  ...
  const downloadTemplate = () => {
    const headers = [
      'cote', 'titre', 'titre_ar', 'auteur', 'type_document', 'langue',
      'annee_publication', 'nombre_pages', 'source_numerisation', 'qualite_numerisation',
      'themes', 'collections', 'niveau_acces', 'telechargement_actif', 'impression_active',
    ];

    const exampleRow = [
      'PHI-2024-001', 'Introduction à la philosophie', 'مقدمة في الفلسفة',
      'Jean Dupont', 'livre', 'fr', '2024', '250', 'internal', 'high',
      'philosophie;méthodologie', 'patrimoine', 'public', 'true', 'true',
    ];
  ...
  const runOcrOnImages = async (images: string[], language: string): Promise<{ pageNum: number; text: string }[]> => {
    ...
    const tesseractLang = language.replace('+', '+');
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center gap-3">
            ...
            <div>
              <CardTitle className="text-2xl">Import en masse des documents</CardTitle>
              <CardDescription>
      ...
      {showResults ? (
        ...
        // Import form view
        <>
          <Tabs defaultValue="combined" className="space-y-6" onValueChange={(v) => setImportMode(v as any)}>
              ...
              <div className="flex justify-end gap-3">
                {importMode === 'metadata' && (
                  ...
                  >
                    ...
                    Importer ({metadataRows.length} fichier(s))
                  </Button>
                )}
