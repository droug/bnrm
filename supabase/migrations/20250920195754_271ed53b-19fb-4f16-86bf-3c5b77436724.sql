-- Amélioration de la sécurité pour la table activity_logs (version corrigée)
...
-- Créer des index pour améliorer les performances (sans fonctions dans les prédicats)
...
CREATE POLICY "No deletes except by admins" 
ON public.activity_logs 
FOR DELETE 
TO authenticated
USING (is_admin_or_librarian(auth.uid()));

-- Anonymiser les données existantes dans la table
UPDATE public.activity_logs 
SET 
  ip_address = anonymize_ip(ip_address),
  user_agent = anonymize_user_agent(user_agent)
