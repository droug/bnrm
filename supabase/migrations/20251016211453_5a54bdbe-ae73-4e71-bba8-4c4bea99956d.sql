...
DROP POLICY IF EXISTS "Everyone can view publishers" ON public.publishers;
DROP POLICY IF EXISTS "Admins can manage publishers" ON public.publishers;
DROP POLICY IF EXISTS "Authenticated users can create publishers" ON public.publishers;
...
CREATE INDEX idx_publishers_name ON public.publishers USING gin(name gin_trgm_ops);
