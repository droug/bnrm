...
DROP TRIGGER IF EXISTS update_program_contributions_updated_at ON public.program_contributions;
DROP TRIGGER IF EXISTS trigger_set_program_contribution_reference ON public.program_contributions;
DROP FUNCTION IF EXISTS set_program_contribution_reference();
DROP FUNCTION IF EXISTS generate_program_contribution_reference();
DROP TABLE IF EXISTS public.program_contributions CASCADE;

-- Create table for cultural program contributions
CREATE TABLE public.program_contributions (
...
CREATE POLICY "Utilisateurs peuvent créer leurs contributions"
  ON public.program_contributions FOR INSERT TO authenticated WITH CHECK (true);

CREATE POLICY "Utilisateurs peuvent voir leurs contributions"
  ON public.program_contributions FOR SELECT TO authenticated
  USING (email = (SELECT email FROM auth.users WHERE id = auth.uid()));

CREATE POLICY "Admins peuvent gérer toutes les contributions"
  ON public.program_contributions FOR ALL TO authenticated
  USING (is_admin_or_librarian(auth.uid()));

-- Functions
CREATE FUNCTION generate_program_contribution_reference()
RETURNS TEXT
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
CREATE FUNCTION set_program_contribution_reference()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
CREATE TRIGGER trigger_set_program_contribution_reference
  ...
  FOR EACH ROW EXECUTE FUNCTION set_program_contribution_reference();
