-- Nettoyage et recréation de la hiérarchie des catalogues
-- CBM → CBN → Digital Library → Manuscripts

-- Nettoyer les tentatives précédentes (au cas où)
DELETE FROM digital_library_documents WHERE is_manuscript = true;
DELETE FROM cbn_documents WHERE document_type = 'Manuscrit';
UPDATE manuscripts SET cbn_document_id = NULL, digital_library_document_id = NULL;

-- Étape 1: Créer des entrées cbn_documents pour chaque manuscrit existant
-- En générant des cotes uniques basées sur l'ID du manuscrit
INSERT INTO cbn_documents (
  id,
  cote,
  title,
  title_ar,
  author,
  document_type,
  support_type,
  physical_status,
  is_digitized,
  access_level,
  publication_year,
  created_at,
  updated_at
)
SELECT 
  gen_random_uuid() as id,
  COALESCE(
    NULLIF(m.cote, ''), 
    'MS-' || substring(m.id::text, 1, 8)
  ) as cote,
  m.title,
  NULL as title_ar,
  m.author,
  'Manuscrit' as document_type,
  'Manuscrit' as support_type,
  CASE m.status::text
    WHEN 'available' THEN 'bon'
    WHEN 'restricted' THEN 'restauration'
    ELSE 'moyen'
  END as physical_status,
  true as is_digitized,
  m.access_level::text as access_level,
  m.publication_year,
  m.created_at,
  m.updated_at
FROM manuscripts m;

-- Étape 2: Mettre à jour les manuscrits avec les cbn_document_id créés
UPDATE manuscripts m
SET cbn_document_id = c.id
FROM cbn_documents c
WHERE c.cote = COALESCE(
    NULLIF(m.cote, ''), 
    'MS-' || substring(m.id::text, 1, 8)
  )
  AND c.document_type = 'Manuscrit';

-- Étape 3: Créer des entrées digital_library_documents pour chaque manuscrit
INSERT INTO digital_library_documents (
  id,
  cbn_document_id,
  title,
  title_ar,
  author,
  document_type,
  language,
  pages_count,
  digitization_quality,
  file_format,
  pdf_url,
  thumbnail_url,
  cover_image_url,
  access_level,
  requires_authentication,
  download_enabled,
  print_enabled,
  is_manuscript,
  manuscript_id,
  publication_status,
  publication_year,
  published_at,
  created_at,
  updated_at
)
SELECT 
  gen_random_uuid() as id,
  m.cbn_document_id,
  m.title,
  NULL as title_ar,
  m.author,
  'Manuscrit' as document_type,
  m.language,
  COALESCE(m.page_count, 0) as pages_count,
  'haute' as digitization_quality,
  'PDF' as file_format,
  m.digital_copy_url as pdf_url,
  m.thumbnail_url,
  m.thumbnail_url as cover_image_url,
  m.access_level::text as access_level,
  CASE m.access_level::text
    WHEN 'public' THEN false
    WHEN 'registered' THEN true
    WHEN 'restricted' THEN true
    ELSE true
  END as requires_authentication,
  COALESCE(m.allow_download, true) as download_enabled,
  COALESCE(m.allow_print, true) as print_enabled,
  true as is_manuscript,
  m.id as manuscript_id,
  'published' as publication_status,
  m.publication_year,
  m.created_at as published_at,
  m.created_at,
  m.updated_at
FROM manuscripts m
WHERE m.cbn_document_id IS NOT NULL;

-- Étape 4: Mettre à jour les manuscrits avec les digital_library_document_id créés
UPDATE manuscripts m
SET digital_library_document_id = d.id
FROM digital_library_documents d
WHERE d.manuscript_id = m.id;

-- Étape 5: Mettre à jour cbn_documents avec les digital_library_document_id
UPDATE cbn_documents c
SET digital_library_document_id = d.id
FROM digital_library_documents d
WHERE c.id = d.cbn_document_id 
  AND c.document_type = 'Manuscrit';

-- Vérification: Afficher les statistiques de la hiérarchie créée
DO $$
DECLARE
  manuscript_count INTEGER;
  cbn_count INTEGER;
  digital_count INTEGER;
  linked_manuscripts INTEGER;
BEGIN
  SELECT COUNT(*) INTO manuscript_count FROM manuscripts;
  SELECT COUNT(*) INTO cbn_count FROM cbn_documents WHERE document_type = 'Manuscrit';
  SELECT COUNT(*) INTO digital_count FROM digital_library_documents WHERE is_manuscript = true;
  SELECT COUNT(*) INTO linked_manuscripts FROM manuscripts 
    WHERE cbn_document_id IS NOT NULL AND digital_library_document_id IS NOT NULL;
  
  RAISE NOTICE '========================================';
  RAISE NOTICE 'HIÉRARCHIE DES CATALOGUES - STATISTIQUES';
  RAISE NOTICE '========================================';
  RAISE NOTICE 'Total manuscrits: %', manuscript_count;
  RAISE NOTICE 'Documents CBN créés: %', cbn_count;
  RAISE NOTICE 'Documents Bibliothèque Numérique créés: %', digital_count;
  RAISE NOTICE 'Manuscrits correctement liés: %', linked_manuscripts;
  RAISE NOTICE '========================================';
END $$;